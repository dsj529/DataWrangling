...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            for field in fields:
                if (line[field]=='') or (line[field]=='NULL'):
                    line[field]=None
                    if not line['name']:
            ...
            if line['synonym']:
                print line['synonym']
                syn_list=line['synonym'].split("|")
                for idx in range(len(syn_list)):
                    syn_list[idx]=re.sub(r"\{|\*|\}", '',syn_list[idx]).strip()
                line['synonym']=syn_list
                
            record={'label' : re.sub(r"\([\w ]*\)", '', line['rdf-schema#label']).strip(),
                    'uri' : line['URI'],
                    'description' : line ['rdf-schema#comment'],
                    'name' : line['name'],
                    'synonym' : line['synonym'],
                    'classification' : {'family': line['family_label'],
